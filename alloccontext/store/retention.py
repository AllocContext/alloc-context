...
def prune_to_horizon(conn: sqlite3.Connection, config) -> dict[str, int]:
    ...
    deleted["alt_quote_snapshots"] = conn.execute(
        "DELETE FROM alt_quote_snapshots WHERE snapshot_ts < ?",
        (ts_cutoff,),
    ).rowcount
    ...
    deleted["onchain_cycle_daily"] = conn.execute(
        "DELETE FROM onchain_cycle_daily WHERE as_of_date < ?",
        (ts_cutoff[:10],),
    ).rowcount
